...
def predict(ctx, query, max_length, top_p, temperature, use_stream_chat, use_internet_search):
    ctx.limit_round()
    flag = True
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    with gr.Column(variant="panel"):
                        with gr.Row():
                            max_length = gr.Slider(minimum=4, maximum=4096, step=1, label='Max Length', value=2048)
                            top_p = gr.Slider(minimum=0.01, maximum=1.0, step=0.01, label='Top P', value=0.1)
                        with gr.Row():
                            temperature = gr.Slider(minimum=0.01, maximum=1.0, step=0.01, label='Temperature', value=0.02)
                        ...
                        with gr.Row():
                            use_internet_search = gr.Checkbox(label='联网搜索', value=False)
                with gr.Row():
                    with gr.Column(variant="panel"):
                        with gr.Row():
